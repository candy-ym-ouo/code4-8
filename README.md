...
验收脚本会真实创建来源、材料、批次、项目、需求、消耗和颜色变化，验证余额从 1000g 降到 500g、撤销后恢复 1000g、最新流水为 `REVERSAL`、颜色和搜索正确；随后继续验证项目状态流转：无有效消耗不能完成项目（阶段门）、回退必须填写原因、退回计划后自动推进不再触发、手动开始后可继续消耗（余额降到 900g）、完成后禁止新增需求、过期版本号被 `VERSION_CONFLICT` 拒绝，且自动推进在流转记录中只出现一次。
...
- 计划用料不扣库存，实际消耗才扣库存。
- 项目状态流转设有阶段门：开始项目前至少有一条材料需求，完成项目前至少有一条未撤销的实际消耗。
- 状态回退（如完成改回进行中）必须填写回退原因，所有流转写入状态流转记录。
- 已完成或已归档项目禁止新增、修改或删除材料需求。
- 项目与材料需求的并发变更按 `version` 乐观锁拒绝覆盖。
- 消耗触发的自动推进（`PLANNED → IN_PROGRESS`）每个项目只触发一次，并有数据库唯一索引兜底。
